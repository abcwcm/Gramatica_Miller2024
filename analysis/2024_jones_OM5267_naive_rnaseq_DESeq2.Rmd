...
```{r decoder}
decoderFile <- "OM5267_naive_decoder.txt"
...
```{r counts}
counts <- read.csv(file = "OM5267_gene_counts.txt", header = TRUE, check.names=FALSE, row.names=1)
...
lapply(rel.contrs, function(x){
  ...
  saveWorkbook(wb = wb, file =  paste0("./differential_expression_res/deseq2_OM5267_naive_",name,"_padj",alphaTR,".res.xlsx"), overwrite=T)
}) %>% invisible()
```
...
```{r volcanoPlot_lfc0.50, fig.width=7, fig.height=5, fig.path='OM5267_pdfs/',  dev=c('png','pdf')}
